...
- [ ] T001 Verify working tree is on branch `042-auth-architecture-redesign` and clean; if dirty, commit or stash (used by all subsequent tasks)
- [ ] T002 [P] Create empty test fixture directories: `tests/fixtures/configs/` and `tests/fixtures/oauth/` (touch a `.gitkeep` if needed so they survive empty-state)
- [ ] T003 [P] Add `tests/integration/conftest.py` shared fixtures: `tmp_mp_home` (sets `MP_CONFIG_PATH` and `HOME` to a tmp dir), `recorded_responses` (httpx_mock or respx), `live_marker` (pytest marker for `MP_LIVE_TESTS=1` opt-in)
- [ ] T004 [P] Add `pytest.ini` markers for `live` and `contract` (used by Research R6 layered tests); update `pyproject.toml` if marker config lives there
- [ ] T005 Add `just` recipe `just test-auth` that runs `pytest tests/unit/test_account.py tests/unit/test_session.py tests/unit/test_resolver.py tests/unit/test_workspace_use.py tests/unit/test_config_v3.py tests/pbt/test_account_pbt.py tests/pbt/test_resolver_pbt.py tests/pbt/test_session_pbt.py -v` for fast iteration on the auth subsystem; verify recipe parses with `just --list`
...
- [ ] T006 [P] Write `tests/unit/test_account.py` covering: ServiceAccount construction with valid/invalid fields, OAuthBrowserAccount construction (no secret in TOML), OAuthTokenAccount XOR(token, token_env) validation, name pattern enforcement (`^[a-zA-Z0-9_-]{1,64}$`), region constraint, frozen/extra=forbid behavior, `auth_header()` for each variant (with stub `TokenResolver`), `is_long_lived()` per variant. Tests MUST fail initially.
- [ ] T007 [P] Write `tests/unit/test_session.py` covering: Project validation (numeric string id), WorkspaceRef validation (positive int id), Session construction with workspace=None, `Session.project_id`/`workspace_id`/`region` properties, `Session.replace(...)` for each axis (preserve other axes; explicit None for workspace clears it; sentinel preserves it). Tests MUST fail.
- [ ] T008 [P] Write `tests/pbt/test_account_pbt.py` (Hypothesis): for every Account variant, round-trip JSON serialization preserves shape; `OAuthTokenAccount` validator never accepts both/neither token+token_env; `Account.name` always satisfies the pattern. Tests MUST fail.
- [ ] T009 [P] Write `tests/pbt/test_session_pbt.py` (Hypothesis): `Session.replace(account=A)` produces a new Session with `account==A` and other axes preserved; `Session.replace(workspace=None)` clears workspace; round-trip serialize/deserialize preserves equality. Tests MUST fail.
- [ ] T010 [P] Write `tests/unit/test_token_resolver.py` covering: `TokenResolver` protocol — `get_browser_token(name, region)` reads from `~/.mp/accounts/{name}/tokens.json` and refreshes if expired; `get_static_token(account)` reads inline `token` or env var via `token_env`; raises `OAuthError` if env var unset; raises `OAuthError` if browser tokens missing/expired/unrefreshable. Tests MUST fail.

### Implementation for Foundational types

- [ ] T011 Create `src/mixpanel_data/_internal/auth/account.py`: `Region` Literal, `AccountType` Literal, `_AccountBase` (frozen, extra=forbid), `ServiceAccount`, `OAuthBrowserAccount`, `OAuthTokenAccount` (with `model_validator(mode="after")` enforcing token XOR), `Account` discriminated union via `Annotated[..., Field(discriminator="type")]`, `TokenResolver` Protocol with `get_browser_token()` and `get_static_token()` methods. Reference [data-model.md §1, §2](data-model.md).
- [ ] T012 [P] Create `src/mixpanel_data/_internal/auth/session.py`: `Project`, `WorkspaceRef`, `Session` (with `replace()` using `_SENTINEL` for workspace), and the convenience properties (`project_id`, `workspace_id`, `region`, `auth_header`). Reference [data-model.md §3, §4](data-model.md).
- [ ] T013 [P] Create `src/mixpanel_data/_internal/auth/token_resolver.py`: `OnDiskTokenResolver` concrete implementation reading from `~/.mp/accounts/{name}/tokens.json` (oauth_browser) and env vars / inline tokens (oauth_token). Includes refresh logic via existing `OAuthFlow.refresh()` (deferred — call into existing module).
- [ ] T014 Update `src/mixpanel_data/_internal/auth/__init__.py` to export `Account`, `AccountType`, `OAuthBrowserAccount`, `OAuthTokenAccount`, `Project`, `Region`, `ServiceAccount`, `Session`, `TokenResolver`, `WorkspaceRef`. Do NOT remove existing exports yet (v2 types coexist until Phase 4).
- [ ] T015 [P] Add new exception classes to `src/mixpanel_data/exceptions.py`: `AccountInUseError(ConfigError)`, `WorkspaceScopeError(MixpanelDataError)`. Keep all existing exception types. Reference [contracts/python-api.md §8](contracts/python-api.md).
- [ ] T016 [P] Add new public types to `src/mixpanel_data/types.py`: `AccountSummary`, `AccountTestResult`, `Target`, `OAuthLoginResult`. Keep existing types (`AccountInfo`, `CredentialInfo`, `ProjectAlias`, `MigrationResult`, `ActiveContext`) — they will be deleted in Phase 4. Reference [data-model.md §5–7](data-model.md).
- [ ] T017 [P] Update `src/mixpanel_data/_internal/auth/storage.py`: add `account_dir(name) -> Path` and `ensure_account_dir(name)` helpers that return/create `~/.mp/accounts/{name}/` with mode `0o700`. Keep existing v2 helpers (`region_token_path`, etc.) — they're used by the v1/v2 paths until Phase 4. Reference [contracts/filesystem-layout.md](contracts/filesystem-layout.md).
- [ ] T018 Run `just test-auth` and confirm all tests T006–T010 now PASS. Run `just typecheck` to confirm no mypy errors in new files.
...
- [ ] T020 [P] [US3] Write `tests/unit/test_resolver.py` covering: account-axis order (env SA quad → env OAUTH_TOKEN → param → target → bridge → `[active].account`); project-axis order (env → param → target → bridge → config); workspace-axis order (same, with None as valid terminal); SA quad takes precedence over OAUTH_TOKEN when both env sets complete (preserves PR #125); `target=` mutually exclusive with `account=`/`project=`/`workspace=` raises `ValueError`; resolver never reads OAuth tokens or hits `/me` (assert via mocks); resolver never mutates `os.environ` (snapshot before/after); `ConfigError` messages match the multi-line format in [contracts/python-api.md §8 / spec FR-024]. Tests MUST fail.
- [ ] T020a [P] [US3] Write `tests/unit/test_settings_headers.py` covering FR-014 + FR-052 in-memory header attachment: a config with `[settings] custom_header = {name = "X-Foo", value = "bar"}` causes `resolve_session()` to produce a Session whose `MixpanelAPIClient` injects the header into outbound requests (assertable via httpx_mock request inspection); same assertions for a bridge file with `headers: {...}`; `os.environ` snapshot before/after `resolve_session()` is byte-identical (no env mutation). Tests MUST fail.
- [ ] T021 [P] [US3] Write `tests/pbt/test_resolver_pbt.py` (Hypothesis) per [research.md R2](research.md): three property classes (determinism, axis independence, env-wins). Use composite strategies for env_state, config_state, explicit_args. Dev profile target <1 s, CI profile <10 s. Tests MUST fail.
...
- [ ] T026 [US3] Create `src/mixpanel_data/_internal/auth/bridge.py` v2 schema only (export/remove come in Phase 8): `BridgeFile` Pydantic model per [data-model.md §10](data-model.md); `load_bridge(path: Path) -> BridgeFile | None` returning None if absent; raise `ConfigError` if malformed; default path resolution (`MP_AUTH_FILE` env, then `~/.claude/mixpanel/auth.json`, then `<cwd>/mixpanel_auth.json`). The resolver in T025 calls `load_bridge()` to populate the bridge axis. The OLD v1 bridge path stays callable via the legacy module name during this phase; only the loader is the new shape — exporting comes later.
- [ ] T027 [US2] Add an `ActiveSession` Pydantic model to `src/mixpanel_data/_internal/auth/session.py` per [data-model.md §5](data-model.md). Used by `ConfigManager.get_active()`.
- [ ] T028 [US2] Update `src/mixpanel_data/_internal/auth/storage.py` to add the migration helper `legacy_token_path(region)` returning `~/.mp/oauth/tokens_{region}.json` (read-only; only used by `mp config convert` later). Keep all existing v1/v2 storage helpers operational.
- [ ] T029 Run `just test-auth` (covers T019–T021) and additionally `pytest tests/unit/cli/test_config_legacy_detection.py -v`. Confirm all tests pass. Run `just typecheck` and `just lint`.
...
- [ ] T042 [US4] Rewrite `src/mixpanel_data/workspace.py` `Workspace.__init__(self, *, account=None, project=None, workspace=None, target=None, session=None)`: route through `resolve_session()` unless `session=` is supplied (full bypass); construct a `MixpanelAPIClient(session=...)`. Add properties `account`, `project`, `workspace`, `session` (all read-only via `@property`; raise on assignment). Add `Workspace.use(self, *, account=None, project=None, workspace=None, target=None, persist=False) -> Self`: validate mutual exclusion; call `_api_client.use(...)`; if `persist=True`, call `ConfigManager.set_active(...)`; return `self`. Add `Workspace.projects(*, refresh=False) -> list[Project]` and `Workspace.workspaces(*, project_all=False, refresh=False) -> list[WorkspaceRef]` calling into `MeService`. Reference [contracts/python-api.md §1–4](contracts/python-api.md).
- [ ] T043 [US4] Update `src/mixpanel_data/_internal/me.py` `MeService` and `MeCache`: storage path moves from `~/.mp/oauth/me_{region}_{name}.json` to `~/.mp/accounts/{name}/me.json`; cache envelope shape per [contracts/filesystem-layout.md §6.4](contracts/filesystem-layout.md). Drop reads/writes of `OAuthTokens.project_id` (legacy field). Test backwards: existing v2 cache files at the old path are NOT auto-migrated — they're ignored on read and a new cache is written to the new path on next access; `mp config convert` handles the move (Phase 10). **Add explicit TTL assertion in `tests/unit/test_me_v3.py` (renamed in T053a) per FR-061: a cache envelope with `fetched_at + 24h < now()` triggers a refetch on next access; a cache envelope with `fetched_at + 24h > now()` is reused without HTTP I/O.**
- [ ] T044 [US4] Update `src/mixpanel_data/_internal/auth/token.py` `OAuthTokens`: REMOVE the `project_id` field (line ~60). Add `extra="ignore"` to gracefully ignore the field if it appears in legacy files (so a half-migrated user is not crashed before they can run `mp config convert`).
- [ ] T045 [US4] Update `src/mixpanel_data/_internal/auth/flow.py`, `client_registration.py`, and `storage.py` for new per-account paths: `OAuthFlow` reads/writes `~/.mp/accounts/{name}/tokens.json`; `client_registration.py` reads/writes `~/.mp/accounts/{name}/client.json`; `storage.py` `account_dir()` and `ensure_account_dir()` are the canonical helpers. Keep the old per-region paths read-callable as `legacy_*` helpers for the conversion script.
- [ ] T046 [US1][US4] Update `src/mixpanel_data/__init__.py` public re-exports per [data-model.md §11](data-model.md): ADD `Account`, `AccountType`, `OAuthBrowserAccount`, `OAuthTokenAccount`, `Region`, `ServiceAccount`, `Project`, `Session`, `WorkspaceRef`, `AccountSummary`, `AccountTestResult`, `Target`, plus the namespaces `accounts`, `targets`, `session`. REMOVE re-exports of `Credentials`, `AccountInfo`, `CredentialInfo`, `AuthCredential`, `CredentialType`, `ProjectContext`, `ResolvedSession`, `ProjectAlias`, `MigrationResult`, `ActiveContext`, `AuthMethod`.
- [ ] T047 [US1][US4] Rewrite `src/mixpanel_data/auth.py` to be a thin re-export module per [data-model.md §11](data-model.md). Remove all v1/v2 type re-exports.
- [ ] T048 [US1][US4] DELETE `src/mixpanel_data/_internal/auth_credential.py` (replaced by `_internal/auth/account.py` + `session.py`). Remove all imports of it across the codebase.
- [ ] T049 [US1][US4] DELETE deprecated types from `src/mixpanel_data/types.py`: `AccountInfo`, `CredentialInfo`, `ProjectAlias`, `MigrationResult`, `ActiveContext`, `AuthMethod`. Keep `AccountSummary`, `AccountTestResult`, `Target`, `OAuthLoginResult` (added in Phase 2).
- [ ] T050 [US1][US4] DELETE deprecated methods on `Workspace`: `set_workspace_id`, `switch_project`, `switch_workspace`, `discover_projects`, `discover_workspaces`, `current_project` property, `current_credential` property, `workspace_id` property, `test_credentials` static method, `_session_to_credentials`. Verify via grep: `grep -rn 'set_workspace_id\|switch_project\|switch_workspace\|discover_projects\|discover_workspaces\|current_project\|current_credential\|test_credentials\|_session_to_credentials' src/` returns zero matches in non-test source.
- [ ] T051 [US1][US4] DELETE the v1/v2 stubs left in `ConfigManager` from T024: any method named `resolve_credentials`, `add_credential`, `set_default`, `set_active_credential`, `set_active_project`, `set_active_workspace`, `set_active_context`, `list_credentials`, `list_project_aliases`, `add_project_alias`, `migrate_v1_to_v2`. Verify via grep.
- [ ] T052 [US1][US4] DELETE the v1/v2 internal `_resolve_session_v1` / `_resolve_session_v2` / `_resolve_region_and_project_for_oauth` and the six-layer fallback in `_internal/config.py`. The single resolver in `_internal/auth/resolver.py` is now the only path.
- [ ] T053 Run `just check` (lint + typecheck + tests). Confirm all tests pass including T030–T037. Confirm `wc -l src/mixpanel_data/_internal/config.py` shows ≤600 LOC. Confirm grep for deleted names returns zero in `src/`.
- [ ] T053a [US1][US4] DELETE obsolete unit tests now that the v1/v2 paths are gone: `tests/unit/test_config_v2.py`, `tests/unit/test_migration.py`, `tests/unit/test_workspace_oauth.py`, `tests/unit/test_auth_credential.py`, `tests/unit/test_auth_bridge.py`, `tests/unit/test_auth_storage.py`. Verify replacement coverage exists: `test_config_v3.py` (T019), `test_workspace_use.py` (T033), `test_account.py` (T006), `test_bridge_v2.py` (T085), `test_session.py` (T007). Also rename surviving v2 tests where the v3 equivalent does not yet exist as a separate file: `tests/unit/test_storage.py` → `tests/unit/test_storage_v3.py` (with content updated for per-account directory layout); `tests/unit/test_me.py` → `tests/unit/test_me_v3.py` (with cache-path + 24h-TTL assertions per FR-061 — see T043). Run `git status` and `grep -rn 'auth_credential\|test_auth_bridge\|test_auth_storage' tests/` returning zero matches confirms cleanup.
...
- [ ] T054 [P] [US5] Write `tests/unit/cli/test_account_cli.py` snapshot tests covering: `mp account list` empty config (multi-line onboarding message); `mp account list` with three accounts (table + json formats); `mp account add` for each `--type`; `mp account add` first-on-fresh-install auto-actives; `mp account remove --force` with orphans; `mp account use NAME`; `mp account show [NAME]`; `mp account test [NAME]` ok/fail; `mp account token [NAME]`; `mp account login NAME` (mocked browser); `mp account logout NAME`. Tests MUST fail. Reference [contracts/cli-commands.md §3](contracts/cli-commands.md).
- [ ] T055 [P] [US5] Write `tests/unit/cli/test_project_cli.py` snapshot tests covering: `mp project list` (with `--remote` and `--refresh`); `mp project use ID`; `mp project show`. Tests MUST fail. Reference [contracts/cli-commands.md §4](contracts/cli-commands.md).
- [ ] T056 [P] [US5] Write `tests/unit/cli/test_workspace_cli.py` snapshot tests covering: `mp workspace list` (with `--project ID`, `--refresh`); `mp workspace use ID`; `mp workspace show` (including "auto-resolved" message when active workspace unset). Tests MUST fail. Reference [contracts/cli-commands.md §5](contracts/cli-commands.md).
- [ ] T057 [P] [US5] Write `tests/unit/cli/test_session_cli.py` snapshot tests covering: `mp session` no args (multi-line summary); `mp session -f json` (matching ActiveSession dump); `mp session --bridge` (when MP_AUTH_FILE set vs unset). Tests MUST fail. Reference [contracts/cli-commands.md §7](contracts/cli-commands.md).
- [ ] T058 [P] [US5] Write `tests/unit/cli/test_globals.py` covering: `--account NAME` overrides for one command; `--project ID` overrides; `--workspace ID` overrides; `--target NAME` overrides; combining `--target` with any of `--account`/`--project`/`--workspace` exits with code 3 + clear message; env vars `MP_ACCOUNT`, `MP_PROJECT_ID`, `MP_WORKSPACE_ID`, `MP_TARGET` work as documented. Tests MUST fail.
- [ ] T058a [P] [US5] Write `tests/unit/cli/test_help_examples.py` per constitution Quality Gate "CLI commands MUST have --help with examples": for every subcommand under `mp account`, `mp project`, `mp workspace`, `mp target`, `mp session`, `mp config`, invoke `mp <group> <verb> --help` via Typer's `CliRunner`; assert the help text contains at least one "Example:" / "Examples:" section AND the example matches a documented command pattern from `contracts/cli-commands.md`. Snapshot golden files in `tests/fixtures/cli_help/`. Tests MUST fail.
- [ ] T059 [P] [US5] Write `tests/unit/cli/test_deprecated_commands_removed.py` covering: every command listed in [contracts/cli-commands.md §9](contracts/cli-commands.md) (e.g., `mp auth list`, `mp projects switch`, `mp context show`, `mp workspaces switch`) returns Typer's "unknown command" error and exits non-zero. Tests MUST fail.

### Implementation for US5

- [ ] T060 [US5] Create `src/mixpanel_data/cli/commands/account.py` Typer app with subcommands `list`, `add`, `remove`, `use`, `show`, `test`, `login`, `logout`, `token`, `export_bridge` (stub for Phase 8), `remove_bridge` (stub). Each subcommand is a thin wrapper over `mp.accounts.*`. Use Rich for table output; respect `-f FORMAT` global. Exit codes per [contracts/cli-commands.md §11](contracts/cli-commands.md).
- [ ] T061 [P] [US5] Create `src/mixpanel_data/cli/commands/project.py` Typer app with subcommands `list` (supports `--remote`, `--refresh`), `use`, `show`. Reference [contracts/cli-commands.md §4](contracts/cli-commands.md).
- [ ] T062 [P] [US5] Create `src/mixpanel_data/cli/commands/workspace.py` Typer app with subcommands `list` (supports `--project ID`, `--refresh`), `use`, `show`. Reference [contracts/cli-commands.md §5](contracts/cli-commands.md).
- [ ] T063 [P] [US5] Create `src/mixpanel_data/cli/commands/session.py` Typer command (single command at the top of the group, with `--bridge` flag). Reference [contracts/cli-commands.md §7](contracts/cli-commands.md).
- [ ] T064 [P] [US5] Create `src/mixpanel_data/cli/commands/config_cmd.py` Typer app with subcommand `convert` as a STUB that exits with `NotImplementedError` for now. The full implementation lands in Phase 10 / US10. Reference [contracts/cli-commands.md §8](contracts/cli-commands.md).
- [ ] T065 [US5] Update `src/mixpanel_data/cli/main.py`: register the new groups (`account`, `project`, `workspace`, `target` (added in Phase 6), `session`, `config`); add the new globals (`--account NAME`/`-a`, `--project ID`/`-p`, `--workspace ID`/`-w`, `--target NAME`); remove `--workspace-id` and `--credential` globals; pass per-command override values into `Workspace.__init__` via the existing `get_workspace()` helper.
- [ ] T066 [US5] Update `src/mixpanel_data/cli/utils.py` `get_workspace()`: read globals from Typer context (or env vars `MP_ACCOUNT`/`MP_PROJECT_ID`/`MP_WORKSPACE_ID`/`MP_TARGET`); enforce target-vs-axis mutual exclusion; pass through to `Workspace(account=..., project=..., workspace=..., target=...)`. Add `format_session(session: ActiveSession, *, with_bridge: bool = False) -> str` for `mp session` output.
- [ ] T067 [P] [US5] Update `src/mixpanel_data/cli/formatters.py`: add formatters `account_summary`, `target`, `session`, `account_test_result` for each output mode (json/jsonl/table/csv/plain).
- [ ] T068 [US5] DELETE `src/mixpanel_data/cli/commands/auth.py`, `projects.py`, `workspaces_cmd.py`, `context.py`. Delete corresponding imports in `cli/main.py`. Delete corresponding tests: `tests/unit/cli/test_auth_cli.py`, `test_projects_cli.py`, `test_workspaces_cli.py`, `test_context_cli.py`.
- [ ] T069 [US5] Update `src/mixpanel_data/cli/CLAUDE.md` to document the new command tree (account/project/workspace/target/session/config), new globals, removed verbs.
- [ ] T070 Run `just check`. Confirm all tests T054–T059 pass. Confirm `mp --help` shows only the new command groups. Confirm `mp auth list` exits non-zero with "unknown command".
