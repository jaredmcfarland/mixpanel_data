...
# Default: 1 sample profile + total count (safe, cheap)
...
print(f"Average LTV: ${result.value:.2f}")
...
result = ws.query_user(distinct_ids=["user_1", "user_2", "user_3"])

# Full-text search
result = ws.query_user(search="alice@example.com", limit=10)
```
