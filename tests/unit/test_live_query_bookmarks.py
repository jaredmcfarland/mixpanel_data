...
Phase 015: Bookmarks API - Tests for query_saved_flows() and related methods.
...
class TestQueryFlows:
    """Tests for LiveQueryService.query_saved_flows()."""

    def test_query_saved_flows_returns_flows_result(self) -> None:
        """query_saved_flows() should return FlowsResult."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
        ...
        result = service.query_saved_flows(bookmark_id=12345)

        assert isinstance(result, FlowsResult)
        assert result.bookmark_id == 12345

    def test_query_saved_flows_parses_steps(self) -> None:
        """query_saved_flows() should parse steps from response."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
            "steps": [
    ...
    def test_query_saved_flows_parses_breakdowns(self) -> None:
        """query_saved_flows() should parse breakdowns from response."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
            "steps": [],
            "breakdowns": [
    ...
    def test_query_saved_flows_parses_conversion_rate(self) -> None:
        """query_saved_flows() should parse overall conversion rate."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
            "steps": [],
            "breakdowns": [],
    ...
    def test_query_saved_flows_parses_computed_at(self) -> None:
        """query_saved_flows() should parse computed_at timestamp."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
            "steps": [],
            "breakdowns": [],
    ...
    def test_query_saved_flows_parses_metadata(self) -> None:
        """query_saved_flows() should parse optional metadata."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
            "steps": [],
            "breakdowns": [],
    ...
    def test_query_saved_flows_empty_metadata_default(self) -> None:
        """query_saved_flows() should default to empty metadata when not present."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
            "steps": [],
            "breakdowns": [],
    ...
    def test_query_saved_flows_calls_api_client(self) -> None:
        """query_saved_flows() should call api_client.query_saved_flows with bookmark_id."""
        mock_api_client = MagicMock()
        mock_api_client.query_saved_flows.return_value = {
            "steps": [],
            "breakdowns": [],
        ...
        service = LiveQueryService(mock_api_client)
        service.query_saved_flows(bookmark_id=12345)

        mock_api_client.query_saved_flows.assert_called_once_with(bookmark_id=12345)
